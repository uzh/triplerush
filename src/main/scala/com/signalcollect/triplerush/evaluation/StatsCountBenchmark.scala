...
object StatsCountBenchmark extends App {
  ...
  /*********/
  def evalName = s"Stats counting."
  def runs = 1
  //  var evaluation = new Evaluation(evaluationName = evalName, executionHost = kraken).addResultHandler(googleDocs)
  var evaluation = new Evaluation(evaluationName = evalName, executionHost = kraken).addResultHandler(googleDocs)
  /*********/

  for (unis <- List(160, 80, 40, 20, 10, 1)) { //10, 20, 40, 80, 160, 320, 480, 800
    for (run <- 1 to runs) {
      for (optimizer <- List(QueryOptimizer.Clever)) {
        for (simulatedMachines <- List(1, 2, 4, 8, 16, 32, 64, 128)) {
