...
object OptimizerTest extends App {
  ...
  /*********/
  def evalName = s"Optimizer permutation test."
  def runs = 1
  //  var evaluation = new Evaluation(evaluationName = evalName, executionHost = kraken).addResultHandler(googleDocs)
  var evaluation = new Evaluation(evaluationName = evalName, executionHost = kraken).addResultHandler(googleDocs)
  /*********/

  for (unis <- List(20, 10)) { //10, 20, 40, 80, 160, 320, 480, 800
    for (run <- 1 to runs) {
  ...
  def lubmBenchmarkRun(
    ...
    val qe = new QueryEngine(graphBuilder, simulatedMachines = simulatedMachines)
    ...
    def runEvaluation(query: QuerySpecification) {
      ...
      val interMachineMessages = qe.getInterMachineMessages
      val hashLookups = qe.getHashLookups
      runResult += s"simulatedMachines" -> simulatedMachines.toString
      runResult += s"interMachineMessages" -> interMachineMessages.toString
      runResult += s"hashLookups" -> hashLookups.toString
      runResult += s"revision" -> revision
      runResult += s"queryId" -> 3.toString
