...
class Sparql11SyntaxTest extends FlatSpec with Matchers {
  ...
  "TripleRush" should "pass syntax tests" in {
    ...
    val subManifests = subManifestsResultSet.map(f => f.get("item").toString).toList.distinct.filter(f => f.endsWith(".ttl"))
    println(subManifests)

    subManifests.map(subManifest => {
      println(subManifest)
      tr.loadNtriples(subManifest)
      tr.prepareExecution
      val query =
        """
          |		PREFIX mf:  <http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#>
          ...
          |		        FILTER(?Type IN (mf:PositiveSyntaxTest11, mf:NegativeSyntaxTest11, mf:PositiveUpdateSyntaxTest11, 
